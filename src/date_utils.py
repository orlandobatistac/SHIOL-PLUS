...
class DateManager:
    ...
    def validate_date_format(cls, date_str: str) -> bool:
        ...
        if date_str.count('-') != 2:
            logger.error("Date validation failed: incorrect format (expected YYYY-MM-DD)")
            return False
    ...
    def get_recent_drawing_dates(cls, count: int = 10) -> List[str]:
        """
        Get dates of upcoming or current drawing dates for polling.
        CRITICAL: Pipeline executes 6 minutes AFTER draw time (11:05 PM ET)
        So if today is a drawing day, we wait for TODAY's results, not tomorrow's.
        
        Logic:
        - If today is a drawing day (Mon/Wed/Sat) → include today (results just published)
        - Continue with future drawing days
        
        Args:
            count: Number of dates to retrieve
            
        Returns:
            List[str]: List of drawing dates in YYYY-MM-DD format, starting with current/next draw
        """
        ...
        if current_weekday in cls.DRAWING_DAYS:
            # Today is a drawing day (Mon/Wed/Sat)
            # Pipeline runs at 11:05 PM, draw was at 10:59 PM
            # Results should be available, add today
            drawing_dates.append(current_date.strftime('%Y-%m-%d'))
            check_date = current_date + timedelta(days=1)
        else:
            # Today is NOT a drawing day
            # Search for next drawing day
            check_date = current_date + timedelta(days=1)

        # Continue searching forward for remaining dates
        ...
        logger.debug(f"Drawing dates for polling: {drawing_dates} (current weekday: {current_weekday})")
        return drawing_dates

    @classmethod
    def get_expected_draw_for_pipeline(cls, last_draw_in_db: Optional[str] = None) -> str:
        """
        Determine which draw date the pipeline should fetch.
        
        Logic:
        1. If last_draw_in_db provided → calculate next expected draw after that
        2. Check if that draw already happened (date < today)
        3. If yes → return that date (missing draw to fetch)
        4. If no → check if today is drawing day and time passed
        5. Return the most recent draw that should exist
        
        Args:
            last_draw_in_db: Last draw date in database (YYYY-MM-DD) or None
            
        Returns:
            str: Expected draw date to fetch (YYYY-MM-DD)
        """
        ...
        # If we have last draw in DB, calculate next expected draw
        if last_draw_in_db:
            try:
                last_draw_date = date_type.fromisoformat(last_draw_in_db)
                
                # Find next drawing day after last_draw_date
                check_date = last_draw_date + timedelta(days=1)
                while check_date.weekday() not in cls.DRAWING_DAYS:
                    check_date += timedelta(days=1)
                
                # If that next draw is in the past or today (and time passed), fetch it
                if check_date < current_date:
                    # Draw already happened, fetch it
                    logger.debug(f"Expected draw: {check_date} (past draw, should exist)")
                    return check_date.strftime('%Y-%m-%d')
                elif check_date == current_date:
                    # Today is drawing day
                    # Check if draw time (10:59 PM) has passed
                    draw_time_today = current_time.replace(
                        hour=cls.DRAWING_HOUR, 
                        minute=cls.DRAWING_MINUTE,
                        second=0,
                        microsecond=0
                    )
                    if current_time >= draw_time_today:
                        # Draw time passed, results should be available
                        logger.debug(f"Expected draw: {check_date} (today, draw time passed)")
                        return check_date.strftime('%Y-%m-%d')
                
            except (ValueError, AttributeError) as e:
                logger.warning(f"Failed to parse last_draw_in_db '{last_draw_in_db}': {e}")
        
        # Fallback: Find most recent drawing day that should have results
        # Go backward from today until we find a drawing day
        check_date = current_date
        
        # If today is a drawing day and time passed, return today
        if current_weekday in cls.DRAWING_DAYS:
            draw_time_today = current_time.replace(
                hour=cls.DRAWING_HOUR,
                minute=cls.DRAWING_MINUTE,
            ...
            if current_time >= draw_time_today:
                logger.debug(f"Expected draw: {current_date} (today is drawing day, time passed)")
                return current_date.strftime('%Y-%m-%d')
        
        # Search backward for most recent drawing day
        check_date = current_date - timedelta(days=1)
        while check_date.weekday() not in cls.DRAWING_DAYS:
            check_date -= timedelta(days=1)
        
        logger.debug(f"Expected draw: {check_date} (most recent past drawing day)")
        return check_date.strftime('%Y-%m-%d')
