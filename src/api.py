...
async def adaptive_learning_update():
    ...
    try:
        ...
        for name, raw in scores.items():
            ...
            confidence = min(0.95, 0.1 + plays / (plays + 100))
...
async def evaluate_predictions_for_draw(draw_date: str):
    ...
    try:
        ...
        for row in preds:
            ...
            # Insert into performance_tracking via helper
            try:
                db.save_performance_tracking(pred_id, draw_date, pred_nums, pred_pb, matches_main, matches_pb, prize_description or 'Non-winning', 0.0, {})
            except Exception:
                # Fallback: raw insert if helper fails
                try:
                    cursor.execute(
                        "INSERT INTO performance_tracking (prediction_id, draw_date, actual_n1, actual_n2, actual_n3, actual_n4, actual_n5, actual_pb, matches_main, matches_pb, prize_tier) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        (pred_id, draw_date, winning_nums[0], winning_nums[1], winning_nums[2], winning_nums[3], winning_nums[4], winning_pb, matches_main, matches_pb, prize_description or 'Non-winning')
                    )
...
async def get_system_overview():
    ...
    try:
        ...
        try:
            ...
            commit_hash = subprocess.check_output(['git', 'rev-parse', '--short=7', 'HEAD'], cwd=os.getcwd(), text=True).strip()
            branch = subprocess.check_output(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], cwd=os.getcwd(), text=True).strip()
            last_message = subprocess.check_output(['git', 'log', '-1', '--pretty=%B'], cwd=os.getcwd(), text=True).strip()
            commit_date = subprocess.check_output(['git', 'log', '-1', '--pretty=%cI'], cwd=os.getcwd(), text=True).strip()
            ...
            git_info.update({
                ...
                'commit_url': f"https://github.com/orlandobatistac/SHIOL-PLUS/commit/{commit_hash}"
            })
