...
def generate_retroactive_predictions(draw_date: str, count_per_strategy: int = 10):
    ...
    with get_db_connection() as conn:
        ...
        # Insert new predictions (using correct column names: strategy_used, confidence_score)
        for ticket in tickets:
            cursor.execute("""
                INSERT INTO generated_tickets
                (draw_date, n1, n2, n3, n4, n5, powerball, strategy_used, confidence_score, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
            """, (
                ticket['draw_date'],
