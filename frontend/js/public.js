    ...
    if (data.predictions && data.predictions.length > 0) {
        let contentHtml = `
            <div class="p-6">
                ...
                <div class="mb-4 p-3 bg-blue-50 rounded-lg">
                    <div class="text-sm text-gray-600 mb-2">Draw Date: <strong>${actualDrawDate}</strong></div>
                    ...
                    ` : '<div class="text-sm text-gray-500">Winning numbers not available</div>'}
                </div>
                <div class="space-y-4">
        `;
        ...
        modalContent.innerHTML = `
            <div class="p-6 text-center">
                ...
                <p class="text-gray-600 mb-2">No prediction data found for draw date: <strong>${actualDrawDate}</strong></p>
                <p class="text-sm text-gray-500 mb-4">${data.message || 'No matches found with the specified criteria'}</p>
        ...
        .catch(error => {
            ...
            modalContent.innerHTML = `
                <div class="p-6 text-center">
                    ...
                    <p class="text-gray-600 mb-2">Unable to load prediction details for this draw.</p>
                    <p class="text-sm text-gray-500 mb-4">Draw date: ${drawDateInput}</p>
                    <p class="text-xs text-gray-400 mb-4">Error: ${errorMessage}</p>
                    <button onclick="closeDrawMatchesModal()" class="mt-4 px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">
                        Close
                    </button>
                </div>
...
async function showDrawMatchesModal(drawDateInput) {
    try {
        ...
        const apiEndpoints = [
            `/api/v1/public/predictions/by-draw/${encodeURIComponent(drawDate)}?min_matches=0&limit=50`,
            `/api/v1/predictions/by-draw/${encodeURIComponent(drawDate)}?min_matches=0&limit=50`,
            `/api/v1/predictions/public/by-draw/${encodeURIComponent(drawDate)}?min_matches=0&limit=50`
        ];
        ...
        if (modalContent) {
            ...
            modalContent.innerHTML = `
                <div class="p-6 text-center">
                    <i class="fas fa-exclamation-triangle text-3xl text-red-500 mb-4"></i>
                    <h3 class="text-lg font-semibold text-red-600 mb-2">Error Loading Data</h3>
                    <p class="text-gray-600 mb-2">Unable to load prediction matches for this draw.</p>
                    <p class="text-sm text-gray-500 mb-4">Draw date: ${drawDateInput}</p>
                    <p class="text-xs text-gray-400 mb-4">Error: ${errorMessage}</p>
                    <div class="space-x-3">
            ...
            <div class="draw-item p-4 mb-4 bg-white rounded-lg shadow hover:shadow-md transition-shadow duration-200 flex justify-between items-center border border-gray-200 cursor-pointer"
                ...
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Draw ID: ${draw.id}</p>
                        <div class="bg-gray-50 p-2 rounded mt-2">
                            ...
                            <div class="flex items-center">
                                <span class="text-xs text-gray-600 mr-2">PB:</span>
                ...
                <span class="text-gray-600 hover:text-blue-500 transition-colors duration-200">
