...
class TicketVerification {
    ...
    async handleFileSelect(event) {
        ...
        if (isMobile && file.size > compressionThreshold) {
            this.showLoading('Optimizing image for mobile...');
            try {
                ...
                this.hideLoading();
                this.showError('Failed to optimize image. Please try a smaller image.');
                return;
            }
        } else if (!isMobile) {
    ...
    displayResults(result) {
        ...
        this.updateTicketDetails(verification);
    }
    ...
    displayWinningPlays(winningPlays) {
        ...
        html += '<h4 class="font-semibold text-green-800 dark:text-green-200 mb-2">Winning Plays:</h4>';
            ...
            html += `
                <div class="bg-white dark:bg-gray-800 rounded-lg p-3 border border-green-200 dark:border-green-700">
                    <div class="flex justify-between items-start mb-2">
                        <span class="font-medium text-green-800 dark:text-green-200">Line ${play.line}</span>
                        <span class="text-green-600 font-bold">$${play.prize_amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    <div class="flex items-center space-x-2 mb-2">
                        ${this.renderNumbers(play.numbers, 'main')}
                        <span class="text-sm text-gray-600 dark:text-gray-400">PB:</span>
    ...
    updateTicketDetails(verification) {
        ...
        this.displayPlayerNumbers(verification.play_results || []);
    }

    displayPlayerNumbers(playResults) {
        const container = document.getElementById('your-plays');
        let html = '';
        
        playResults.forEach(play => {
            const isWinner = play.is_winner;
            const borderClass = isWinner ? 'border-green-300 dark:border-green-600 bg-green-50 dark:bg-green-900/20' : 'border-gray-200 dark:border-gray-600';
            
            html += `
                <div class="flex items-center justify-between p-3 rounded-lg border ${borderClass} mb-2">
                    <div class="flex items-center space-x-2">
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Line ${play.line}:</span>
                        ${this.renderNumbers(play.numbers, 'main')}
                        <span class="text-sm text-gray-600 dark:text-gray-400">PB:</span>
                        ${this.renderNumbers([play.powerball], 'powerball')}
                    </div>
                    <div class="text-right">
                        ${isWinner ? 
                            `<span class="text-green-600 font-bold text-sm">$${play.prize_amount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>` :
                            `<span class="text-gray-500 text-sm">No Prize</span>`
                        }
                    </div>
                </div>
            `;
        ...
        const labelMatch = existingContent.match(/^(.+?Your Numbers:.+?)<\/p>/);
        if (labelMatch) {
            container.innerHTML = labelMatch[0] + '<div class="mt-2">' + html + '</div>';
        } else {
            container.innerHTML += '<div class="mt-2">' + html + '</div>';
        }
    }
    ...
    renderNumbers(numbers, type) {
        ...
        const baseClass = 'inline-flex items-center justify-center w-8 h-8 text-sm font-bold rounded-full';
        const typeClass = type === 'powerball' ? 
            'bg-red-600 text-white' : 
            'bg-white text-gray-900 border-2 border-gray-400';
        
        return numbers.map(num => 
            `<span class="${baseClass} ${typeClass}">${num}</span>`
        ).join('');
    }
    ...
    displayNumbersPreview(result) {
        ...
        if (detectedPlays.length < 3) {
            ...
            warningDiv.className = 'mb-3 p-3 bg-yellow-50 dark:bg-yellow-900/30 rounded border border-yellow-200 dark:border-yellow-800';
            warningDiv.innerHTML = `
                <div class="flex items-start">
                    <i class="fas fa-exclamation-triangle text-yellow-600 mr-2 mt-1"></i>
                    <div class="flex-1">
                        <h4 class="font-semibold text-yellow-800 dark:text-yellow-200 text-sm">Incomplete Detection</h4>
                        ...
                        <button onclick="ticketVerification.showManualPlayEntry()" 
                            class="mt-2 px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            <i class="fas fa-edit mr-1"></i>
        ...
        detectedPlays.forEach(play => {
            ...
            playDiv.className = `flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded border ${play.is_valid ? 'border-green-200 dark:border-green-800' : 'border-red-200 dark:border-red-800'}`;
            
            playDiv.innerHTML = `
                <div class="flex items-center">
                    <span class="font-semibold text-sm mr-3 ${play.is_valid ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}">
                        Play ${play.play_letter}:
                    </span>
                    <div class="flex items-center space-x-1">
                        ${play.main_numbers.map(num => `
                            <span class="inline-flex items-center justify-center w-6 h-6 text-xs font-bold bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full">
                                ${num}
                            </span>
                        `).join('')}
                        <span class="mx-2 text-gray-400">PB:</span>
                        <span class="inline-flex items-center justify-center w-6 h-6 text-xs font-bold bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200 rounded-full">
                            ${play.powerball}
                        </span>
                    </div>
                </div>
                <div>
                    ${play.is_valid ? 
                        ...
                        '<i class="fas fa-exclamation-triangle text-red-500" title="Invalid numbers"></i>'
                    }
                </div>
            `;
