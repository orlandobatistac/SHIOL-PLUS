            ...
            <div class="flex items-center justify-between p-3 rounded-lg border ${bgClass}">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-white/60">${info.count} ${info.count === 1 ? 'outcome' : 'outcomes'}</div>
                    </div>
                </div>
    ...
    try {
        ...
        if (data.prize_tiers && Object.keys(data.prize_tiers).length > 0) {
            ...
            if (tiersWithWins.length > 0) {
                bestMatchTier = abbreviatePrizeTierLabel(tiersWithWins[0][0]);
            }
        }
        if (bestMatchEl) bestMatchEl.textContent = bestMatchTier;
        if (totalPrizesEl) totalPrizesEl.textContent = formatCurrency(data.total_prize || 0);

        // Row 3: Smart Outcomes with Matches (Winners Only)
        ...
        if (tbody) {
            const predictions = Array.isArray(data.top_predictions) ? data.top_predictions : (data.predictions || []);
            ...
            if (winners.length === 0) {
                ...
                winners.forEach((p, idx) => {
                    ...
                    // Si hay match, resalta igual que las blancas pero mantiene fondo rosado y agrega aro verde
                    const pbClass = pbMatch
                        ? 'num-chip powerball-chip ring-2 ring-green-400/70 shadow-[0_0_10px_rgba(74,222,128,0.5)]'
                        : 'num-chip powerball-chip';
                    
                    tr.innerHTML = `
                        <td class="px-4 py-3 text-white/60 font-medium">#${idx + 1}</td>
                        <td class="px-4 py-3">
