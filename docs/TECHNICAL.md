...
**Dual Table System for Ticket Storage:**

SHIOL+ uses two separate tables for ticket storage, each serving a distinct purpose:

#### `generated_tickets` - Pipeline v1 Predictions (Official)

- **Purpose**: Stores official predictions generated by Pipeline STEP 6
- **Source**: `StrategyManager.generate_balanced_tickets()` (6 v1 strategies)
- **Quantity**: 200 tickets per pipeline execution
- **Key Features**:
  - Linked to specific `draw_date` (next upcoming draw)
  - Evaluable against official results after draw occurs
  - Strategy attribution (`strategy_used` column)
  - Confidence scoring per ticket
  - Enforces sorted unique white-balls and PB range (1-26)
- **Use Case**: Public prediction API endpoints, performance evaluation, adaptive learning
...
CREATE TABLE generated_tickets (
    ...
    n1, n2, n3, n4, n5 INTEGER NOT NULL,
...
#### `pre_generated_tickets` - ML Cache (High-Speed Retrieval)

- **Purpose**: Background-generated ML ticket cache for ultra-fast API responses
- **Source**: `BatchTicketGenerator` (background thread after pipeline)
- **Quantity**: 100 tickets per mode (RandomForest, LSTM)
- **Key Features**:
  - NOT linked to specific draw_date (generic pool)
  - Optimized for <10ms retrieval speed
  - Mode-based classification (`mode` IN 'random_forest', 'lstm', 'v1', 'v2', 'hybrid')
  - Metadata JSON with generation timestamp
  - Auto-cleanup after 7 days
- **Use Case**: Premium API endpoints requiring instant response, batch ticket retrieval
...
**Architecture Decision**: The dual-table approach separates concerns:

- `generated_tickets` → Temporal, evaluable, tied to specific draw
- `pre_generated_tickets` → Stateless, fast, generic ticket pool

**Related Documentation**: See `docs/BATCH_GENERATION.md` for complete batch system architecture.
...
3. Expected frequency per pair is computed as the combinatorial probability of both numbers appearing in a draw. The implementation uses:
   ...
   expected_per_pair = (5/69) _ (4/68) _ total_draws
...
**Medium Priority:** 2. **Improvement 3: Temporal Weighting** (good balance)

- Moderate complexity
- Improves existing system
- Compatible with current architecture

**~~Low Priority:~~ COMPLETED:** 3. ~~**Improvement 1: XGBoost Integration**~~ ✅ **DONE (Nov 2025)**
